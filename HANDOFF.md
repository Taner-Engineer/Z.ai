...
- Архитектура 2026-09-14: **локальный быстрый путь** `win-worker/run_local.py`.
  Файлы, лежащие в `_Drop` рабочей копии ваулта на рабочем ПК, обрабатываются
  СРАЗУ локально (worker.py, RTX 3050), на homelab уезжают только манифесты +
  готовые `output.md` в `_Drop/_results/` (демон доводит заметки как обычно).
  Исходники из `_Drop` переносятся в `C:\Users\Us\2brain-worker\drop-archive\<дата>\`
  (вне ваулта — Syncthing разносит удаление, демон homelab не ставит дубль).
  Заметки-заглушки run_local создаёт сам (queued-gpu), манифесты — по схеме
  gpu_queue.enqueue. DJVU/видео — по-прежнему старым путём через homelab.
  Старый путь `run_queue.py` не тронут (homelab-очередь и vast.ai).
  Канал ПК↔homelab ~4.5 МБ/с: 1.1 ГБ входов тянулись 4+ мин — из-за этого
  таймауты tar в run_queue подняты до 900 с (b9a31d0).
...
## Правило синхронизации пайплайна (от дрейфа)

Любая сессия, правящая код пайплайна, коммитит в ОБЕ стороны:
1. `/opt/2brain` на homelab (там живёт демон) — commit;
2. локальный `~/zcode-sync/2brain/` + push в GitHub Z.ai.
Дрейф чинится сверкой md5 и подтягиванием более свежей стороны.
