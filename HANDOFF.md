...
- Очередь исполняется на рабочем ПК ПРЯМО СЕЙЧАС: `run_queue.py` (лог
  `C:\Users\Us\2brain-worker\run2.log`). СП 252 и СП 56 — done; Уманский
  (1043 стр., OCR, ~15–35 мин) в работе. Раннер сам вернёт результаты на
  homelab в `_Drop/_results/`, демон сам доведёт заметки до queued —
  ЭТО НЕ ТРЕБУЕТ АГЕНТА, ничего не трогать до «готово» в логе.
- Рабочий ПК — всегда-доступный узел: сон/гибернация отключены, OpenSSH Server
  ...
  (сборка LTSC) — перезагрузить ПК ПОСЛЕ завершения очереди, иначе потеряются
  результаты раннера.
